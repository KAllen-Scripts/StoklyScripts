...
async function generateCSV(channelID){
    let csvArr = []
    await common.loopThrough('Getting channel and item Data', `https://api.dev.stok.ly/v0/channels/${channelID}/listings`, 'size=1000', '[status]!={2}', async (listing)=>{
        let listingData = await common.requester('get', `https://api.dev.stok.ly/v0/listings/${listing.listingId}`).then(r=>{return r.data.data})
        let itemData = await common.requester('get', `https://api.dev.stok.ly/v0/items/${listing.itemId}`).then(r=>{return r.data.data})
...
(async ()=>{
    ...
    let attributeDict = {}
    await common.loopThrough('Getting attribute lookup', `https://api.dev.stok.ly/v0/item-attributes`, 'size=1000', '[status]!={1}', (att)=>{
    ...
    await common.loopThrough('Updating Listings', `https://api.dev.stok.ly/v0/channels/${channelID}/listings`, 'size=1000', '[status]!={2}', async (listing)=>{
        ...
        if(removeAll){
            await common.requester('patch', `https://api.dev.stok.ly/v0/listings/${listing.listingId}`, {data:patchData})
            return
        }
